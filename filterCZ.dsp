...
process(x) = envPitch(thresholdSlider, x)~_<:(attackReplacer(x)+synthLevel*CZsynth)*totalLevel;

attackReplacer(x, env, pitch) = (x/max(ma.EPSILON, env))*attackEnv(env);

synthLevel(env, pitch) = (1-attackEnv(env))*0.6;

// remap(from1, from2, to1, to2, x)

xfadeSelectorOLD(sel, nr) = ((sel<=nr)*((sel-nr)+1):max(0))+((sel>nr)*((nr-sel)+1):max(0));

xfadeSelector(sel, nr) = (1-abs(sel-nr)):max(0);

totalLevel(env, pitch) = (env/0.6)*xfadeSelector(levelMod, 0)+blockLevel(env)*xfadeSelector(levelMod, 1)+xfadeSelector(levelMod, 2);

blockLevel(env) = it.remap(thresholdSlider*0.5, thresholdSlider, 0, 1, env:max(thresholdSlider*0.5):min(thresholdSlider));

CZsynth(env, pitch) = phasesAndFilters:oscillators(index):si.interpolate(octave:ma.frac)
    with {
        fund = os.lf_sawpos(pitch*oct2mult(minOctave));
        f0 = (fund*oct2mult(floor(octave)-minOctave)):ma.frac;
        f1 = f0*2:ma.frac;
        phasesAndFilters = (f1, f0):swap(bool):(preFilter, preFilter):swap(1-bool);
        swap(bool, a, b) = select2(bool, a, b), select2(bool, b, a);
        bool = floor(abs(octave-minOctave))%2;
        oscillators(index, f0, f1) = os.CZsquare(f0, index), os.CZsquare(f1, index);
        preFilter(x) = (ve.korg35LPF(filterFreq:max(0):min(Fthres), Q, x), x):si.interpolate(filterBP);
        Fthres = 0.95;
...
attackEnv(env) = (loop~_)*attackReplace
    with {
        trig = env>thresholdSlider:ba.impulsify;
        loop(prevEnv) = ((prevEnv<1)&(prevEnv>0)&(prevEnv>prevEnv'))|trig:smootherARorder(4, 4, 2, attack, decay);
    };
