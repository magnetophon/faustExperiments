...
,
  (x:LAreleaseBlock(LA,holdTimeX)~_)
   ...
   with {
     ...
     // counting = (hold(LA,attackHold(LA),x)~_) == (hold(LA,attackHold(LA),x)~_)';
     // counting = (hold(LA,holdTime,x)~_) >= (x@(lookahead(LA)));
     counting = (x:hold(LA,holdTime)~_) == (x:hold(LA,holdTime)~_)';
     // 0.0001 ;
